...
export default function TvshowsDetails() {
    ...
    let getDetails = async () => {
      let { data } = await axios.get(
        `https://api.themoviedb.org/3/tv/${params.id}?api_key=c2db102a90b57fa5e0517e57a07ffe3f&language=en-US`
      );
      console.log(data);
      setDetails(data);
    };
            ...
            >
              ...
              <div className={`${style.text}`}>
                ...
                details.genres.map(genre=>(
                  <span className="me-1" style={{fontSize:"15px",} } key={genre.id}>{genre.name} •</span>
                )):" "}
                 </span>
                  {details.first_air_date} • {details.number_of_episodes} Episodes  •  {details.number_of_seasons} Season •  Status: {details.status} 
                  <br/>{details.tagline ? details.tagline : "no added tag line"} <br/>
                  </p>
                  <h3
                  style={{
                    fontWeight:'700'
